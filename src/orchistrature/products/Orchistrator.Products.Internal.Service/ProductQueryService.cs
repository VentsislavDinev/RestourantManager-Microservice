...
namespace WiLSoft.Automation.RestourantManager
{
    public class ProductQueryService : ApplicationService, IProductQueryService
    {
        ...
        public async System.Threading.Tasks.Task<ProductsDto> Get(int model)
        {
            ...
            return JsonConvert.DeserializeObject<ProductsDto>(responseString);
        }

        public async System.Threading.Tasks.Task<PagedResultDto<ProductsDto>> GetAll()
        {

            var response = await _apiClient.GetAsync("");
            var responseString = await response.Content.ReadAsStringAsync();

            return JsonConvert.DeserializeObject<PagedResultDto<ProductsDto>>(responseString);
        }
    }
}
